...
Decision: infrastructure is defined with **AWS CDK (Python)**, living in its own `cdk/` directory as a separate `uv`-managed project (own `pyproject.toml`/`uv.lock`) — it depends on `aws-cdk-lib`/`constructs`, which have no reason to be installed alongside the Flask app's runtime dependencies. The app is split into five stacks — `PastesDataStack`, `PastesNetworkStack`, `PastesComputeStack`, `PastesWafStack`, `PastesEdgeStack` — mirroring the architecture already decided in `ADR-002` (DynamoDB) and `ADR-005` (Fargate + ALB + CloudFront), so each stack can be built, tested, and reviewed independently while `app.py` wires them together via cross-stack references.
